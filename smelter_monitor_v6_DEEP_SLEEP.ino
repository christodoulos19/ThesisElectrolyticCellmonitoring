// ============================================================
// Aluminum Smelter Brick Monitor  –  v6 (Deep Sleep Edition)
...
//
// ── Deep Sleep Flow ─────────────────────────────────────────
//   Wake → setup() → sensor init → WiFi (Static IP) → takeReading()
//        → loop() handles web clients for ACTIVE_WINDOW_MS (10s)
//        → Deep Sleep for SLEEP_DURATION_US (10 min)
//   RTC Slow Memory retains history, stats & baseline across cycles.
// ============================================================
...
// ── Deep Sleep timing ────────────────────────────────────────
#define ACTIVE_WINDOW_MS   10000UL              // 10 seconds active
#define SLEEP_DURATION_US  ((9ULL * 60) + 50) * 1000000 // 9 minutes and 50s in µs
...
IPAddress STATIC_IP (192, 168, 1, 100);  // Σταθερή IP του ESP32
IPAddress GATEWAY   (192, 168, 1,   1);  // Gateway (router)
IPAddress SUBNET    (255, 255, 255, 0);  // Subnet mask
IPAddress DNS1      (8,   8,   8,   8);  // Primary DNS (Google)
 
// WiFi σύνδεση timeout: αν δεν συνδεθεί σε τόσο ms, πάμε σε AP mode
...
// ύπνου ορίζονται με RTC_DATA_ATTR ώστε να αποθηκεύονται στη
// RTC Slow Memory που δεν διαγράφεται κατά τον Deep Sleep.
 
RTC_DATA_ATTR Reading history[HISTORY_SIZE];
RTC_DATA_ATTR int   historyIndex = 0;
RTC_DATA_ATTR int   historyCount = 0;
RTC_DATA_ATTR int   totalCount   = 0;
 
RTC_DATA_ATTR float baselineZ   = 0;
RTC_DATA_ATTR bool  hasBaseline = false;
 
RTC_DATA_ATTR float minZ  = 1e9, maxZ  = 0, sumZ  = 0;
RTC_DATA_ATTR float minTH = 1e9, maxTH = 0, sumTH = 0;
RTC_DATA_ATTR float minTC = 1e9, maxTC = 0, sumTC = 0;
...
// Χρονόμετρο για το active window (αρχικοποιείται στο setup)
...
float measureImpedance(float &vb, float &vt) {
  ...
  vb = readMV(MCP342x::channel1);
  vt = readMV(MCP342x::channel2);
 
  dacWrite(DAC_PIN, 0);
 
  float vRef = vt - vb;
  if (abs(vRef) < 0.01)  return 9999999.0;
  if (abs(vb)   < 0.001) return 0.0;
...
void takeReading() {
  ...
  // Auto-baseline μετά από 5 μετρήσεις
...
function update(d) {
  ...
  battEl.className = d.batt < 20 ? 'card-value batt-low' : 'card-value batt-good';
  ...
  if (d.tHot > -900 && d.tCold > -900) {
    document.getElementById('dtVal').textContent = (d.tHot - d.tCold).toFixed(1) + ' °C';
  }
...
function drawChartZ() {
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    ctx.fillText(fmtZ(maxV - (maxV - minV) * i / 4), pad.left - 6, y + 4);
  }
 
  ctx.strokeStyle = '#ff6b1a'; ctx.lineWidth = 2; ctx.beginPath();
  let first = true;
...
function drawChartT() {
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    ctx.beginPath(); ctx.moveTo(pad.left, y); ctx.lineTo(pad.left + cW, y); ctx.stroke();
    ctx.fillStyle = '#888'; ctx.font = '11px Arial'; ctx.textAlign = 'right';
    ctx.fillText((maxV - (maxV - minV) * i / 4).toFixed(0) + '°C', pad.left - 6, y + 4);
  }
 
  // Hot line
  ctx.strokeStyle = '#ff3d3d'; ctx.lineWidth = 2; ctx.beginPath();
  let first = true;
  ...
  // Legend
  ctx.fillStyle = '#ff3d3d'; ctx.fillRect(pad.left, H - 20, 12, 12);
  ctx.fillStyle = '#e8e8ea'; ctx.font = '12px Arial'; ctx.textAlign = 'left';
  ctx.fillText('Hot Face', pad.left + 18, H - 10);
  ctx.fillStyle = '#00d4ff'; ctx.fillRect(pad.left + 100, H - 20, 12, 12);
  ctx.fillText('Cold Face', pad.left + 118, H - 10);
}
 
function fetchData() {
  fetch('/data').then(r => r.json()).then(d => update(d)).catch(e => console.error(e));
}
 
function setBaseline() { fetch('/setbaseline').then(() => fetchData()); }
 
function reset() {
  if (confirm('Reset all data?')) fetch('/reset').then(() => fetchData());
}
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n=== Aluminum Smelter Monitor (v6 – Deep Sleep) ===");
  Serial.printf("Wake-up #%d | History: %d entries\n", totalCount, historyCount);
  ...
  // Ρύθμιση στατικής IP πριν την αποστολή WiFi.begin()
  // ώστε να παραλειφθεί το αργό DHCP handshake.
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Fallback: Access Point mode αν η σύνδεση απέτυχε
    Serial.println("\nWiFi failed – starting Access Point.");
    WiFi.softAP("SmelterMonitor", "monitor123");
    Serial.print("AP IP: "); Serial.println(WiFi.softAPIP());
  }
 
  // ── Μέτρηση αισθητήρων (εφάπαξ ανά κύκλο αφύπνισης) ────
  takeReading();
...
void loop() {
  ...
  if (millis() - wakeTime >= ACTIVE_WINDOW_MS) {
    ...
    Serial.flush();   // Εξασφαλίζουμε ότι το Serial έχει τυπωθεί πριν τον ύπνο
